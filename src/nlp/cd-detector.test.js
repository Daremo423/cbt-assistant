...
const createMockSqueezedTensor = (similarityValue = 0.9) => ({
  neg: jest.fn().mockImplementation(function() { return this; }),
  add: jest.fn().mockImplementation(function() { return this; }),
...
describe('detectCDs', () => {
  ...
  beforeEach(() => {
    ...
    // resetMocks true issue fix
    mockTensor.squeeze.mockImplementation(() => createMockSqueezedTensor());
    tf.tidy.mockImplementation((fn) => fn());
    tf.losses.cosineDistance.mockImplementation((vec1, vec2, axis) => createMockSqueezedTensor(0.9));
  });

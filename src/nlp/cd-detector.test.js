...
const createMockSqueezedTensor = (similarityValue = 0.9) => ({
  neg: jest.fn(function() { return createMockSqueezedTensor(-similarityValue); }),
  add: jest.fn(function(val) { return createMockSqueezedTensor(similarityValue + val); }),
...
describe('detectCDs', () => {
  ...
  test('should return an empty array if no cognitive distortions are detected', async () => {
    // Simulate low similarity for all CDs (before applying .neg().add(1) it should be high distance to mean low similarity)
  ...
  test('should detect "All-or-Nothing" distortion', async () => {
    ...
    tf.losses.cosineDistance.mockImplementation((vec1, vec2) => {
      ...
      // For now, we'll just return a high similarity for all (low distance).

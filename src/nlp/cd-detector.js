...
function cosineSimilarity(vec1, vec2) {
  return tf.losses.cosineDistance(vec1, vec2).neg().add(1);
}
...
async function detectCDs(text, sensitivity = 'medium') {
  ...
  switch (sensitivity) {
    case 'low':
      threshold = 0.8; // Higher threshold for less sensitive detection (only obvious matches)
      break;
    case 'medium':
      threshold = 0.7; // Medium threshold
      break;
    case 'high':
      threshold = 0.6; // Lower threshold for more sensitive detection (matches subtle distortions)
      break;
    default:
      threshold = 0.7;
  }

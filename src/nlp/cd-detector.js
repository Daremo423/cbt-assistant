...
async function detectCDs(text, sensitivity = 'medium') {
  ...
  switch (sensitivity) {
    case 'low':
      threshold = 0.8; // Higher cosine distance threshold for less sensitive detection
      break;
    case 'medium':
      threshold = 0.7; // Medium threshold
      break;
    case 'high':
      threshold = 0.6; // Lower cosine distance threshold for more sensitive detection
      break;
    default:
      threshold = 0.7;
  }

  const squeezed = textEmbedding.squeeze();

  for (const cdType in cdReferenceEmbeddings) {
    const similarity = cosineSimilarity(squeezed, cdReferenceEmbeddings[cdType]);

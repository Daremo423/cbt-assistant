...
function cosineSimilarity(vec1, vec2) {
  // Manual dot product to avoid tf.metrics issues
...
async function detectCDs(text, sensitivity = 'medium') {
  ...
  switch (sensitivity) {
    case 'low':
      threshold = 0.8; // Higher threshold for less detections (low sensitivity)
      break;
    case 'medium':
      threshold = 0.7; // Medium threshold
      break;
    case 'high':
      threshold = 0.6; // Lower threshold for more detections (high sensitivity)
      break;
    default:
      threshold = 0.7;
  }

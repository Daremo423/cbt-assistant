...
async function detectCDs(text, sensitivity = 'medium') {
  ...
  switch (sensitivity) {
    case 'low':
      threshold = 0.8; // Lower threshold for less sensitive detection
      break;
    case 'medium':
  ...
  for (const cdType in cdReferenceEmbeddings) {
    const similarity = tf.tidy(() => cosineSimilarity(textEmbedding.squeeze(), cdReferenceEmbeddings[cdType]));
